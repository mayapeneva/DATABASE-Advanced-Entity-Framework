namespace MiniORM
{
    ...
    public abstract class DbContext
    {
        ...
        protected DbContext(string connection)
        {
            ...
            where TDbSet : class, new() where TCollection : class, new()
        {
            ...
            var primaryKeys = collectionType.GetProperties().Where(p => p.HasAttribute<KeyAttribute>()).ToArray();
        ...
        {
            var entityType = typeof(TEntity);
            var foreignKeys = entityType.GetProperties().Where(p => p.HasAttribute<ForeignKeyAttribute>()).ToArray();
            foreach (var foreignKey in foreignKeys)
            {
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    var navigationPropertyValue =
                        ((IEnumerable<object>)navigationDbSet).First(np =>
                           navigationPrimaryKey.GetValue(np).Equals(foreignKeyValue));
        ...
        {
            var tableName =
                ((TableAttribute)Attribute.GetCustomAttribute(tableType, typeof(TableAttribute)))?.Name;
            if (tableName == null)
            {
